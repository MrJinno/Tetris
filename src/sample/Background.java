...
public class Background extends Pane {
    ...
    private int maxX=(int)(BOARD_WIDHT/ SQUARE_HEIGH);
    private int maxY=(int)(BOARD_HEIGH/ SQUARE_HEIGH);
    private Square shape;
    private Scene scene;
    private Square[][] board=new Square[(int)(BOARD_HEIGH/ SQUARE_HEIGH)][(int)(BOARD_WIDHT/ SQUARE_HEIGH)];
    ...
    for (int i=minY;i<board[0].length-2;i++){
        ...
        board[minY][i].getRectangle().setX(STARTING_X+(SQUARE_HEIGH*i)+ SQUARE_HEIGH);
        ...
        board[maxY-1][i].getRectangle().setX(STARTING_X+(SQUARE_HEIGH*i)+ SQUARE_HEIGH);
